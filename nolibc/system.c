...
sPtr SysWrite(const u64 fd, const u8* buf, const u64 count) {
	return SysCall(SYS_write, fd, (uPtr)buf, count, 0, 0, 0);
}

sPtr SysRead(const u64 fd, u8* buf, const u64 count) {
  return SysCall(SYS_read, fd, (uPtr)buf, count, 0, 0, 0);
}
...
sPtr SysOpen(const char* pathname, const u64 flags, const u64 mode) {
  return SysCall(SYS_open, (uPtr)pathname, flags, mode, 0, 0, 0);
}

sPtr SysClose(const u64 fd) {
  return SysCall(SYS_close, fd, 0, 0, 0, 0, 0);
}

sPtr SysLSeek(const u64 fd, const u64 offset, const u64 whence) {
  return SysCall(SYS_lseek, fd, offset, whence, 0, 0, 0);
}

sPtr SysFStat(const u64 fd, u8* buf) {
  return SysCall(SYS_fstat, fd, (uPtr)buf, 0, 0, 0, 0);
}
